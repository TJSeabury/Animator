 ...
 * @type Number
 */
export function clamp( min :number = -1, max :number = 1 ) :number {
 ...
 * @returns 
 */
export function sminCubic( a :number, b :number, k :number ) {
...
export function generateCubicBezierCurve( a={x:0,y:0}, b={x:0,y:1}, c={x:1,y:1}, d={x:1,y:0} ) {
    return function curve( t ) {
        let x = ( 1-t )**3 * a.x + 3 * ( 1-t )**2 * t * b.x + 3 * ( 1-t ) * t**2 * c.x + t**3 * d.x;
        let y = ( 1-t )**3 * a.y + 3 * ( 1-t )**2 * t * b.y + 3 * ( 1-t ) * t**2 * c.y + t**3 * d.y;
        return {
            x: x,
            y: y
        };
...
export function generateRandomNumberInclusiveInverseQuadratic( min, max ) {
    const curve = generateQuadraticBezierCurve( 
        {x: min, y: max}, 
        {x: min, y: min}, 
        {x: max, y: min} 
    );
...
export function relativePositionToViewport( element :HTMLElement ) :number {
    if ( ! element ) { return; }
...
export function relativeSizeToViewport( element :HTMLElement ) :number {
    if ( ! element ) { return; }
    let rect = element.getBoundingClientRect();
